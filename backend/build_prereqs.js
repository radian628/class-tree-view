...
import { getPrereqsRaw, getPrereqs, getPrereqsSyntaxTree } from '../scraper/get-prereqs.js';
...
async function convertPrereqToCourse(subject, courseNumber){
...
async function populatePrereqs(course) {
    ...
    const prereqs = await getPrereqs(courseInfo.term, courseInfo.courseReferenceNumber);
    // Build infix expression
    let prereqString = course;
    if (!prereqs) {
        return prereqString;
    }
    else if (prereqs.length == 1) { // This hasn't happened in testing but I imagine it will for other subjects?
        return prereqString;
    }
    prereqString += "<(";
    for (let requirement of prereqs) {
        let requirementString = "";
        requirementString += (requirement.andOr === 'And' ? '&' : (requirement.andOr === 'Or' ? '|' : ''));
        requirementString += requirement.openParen;
        let course = await convertPrereqToCourse(requirement.subject, requirement.number);
...
async function main() {
    ...
    CREATE TABLE IF NOT EXISTS Requirements (
        ...
        booleanType ENUM('and', 'or', 'requires')
    )`;
